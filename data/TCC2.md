...
A divisão temporal fixa na proporção 80/20 não permite validação cruzada em múltiplas janelas temporais (time series cross-validation), limitando a robustez da avaliação a um único período de teste. Esta abordagem não quantifica variabilidade de desempenho quando modelos são aplicados a diferentes períodos históricos. Adicionalmente, a seleção de hiperparâmetros do XGBoost foi realizada empiricamente baseada em práticas estabelecidas e ajustes manuais, em vez de busca sistemática exaustiva (grid search ou random search), podendo não representar a configuração verdadeiramente ótima para este dataset específico.
